import { ForbiddenException, Injectable } from '@nestjs/common';
import {
  AccessResolver,
  ResolvedAudience,
} from '../contracts/access-resolver.contract';
import { SectionProvider } from '../contracts/section-provider.contract';
import { RegisterProvider } from '../registry/register-provider.decorator';
import { RequestHistorySectionEntity } from './entities/request-history-section.entity';
import { ResourcingService } from './resourcing.service';

@Injectable()
@RegisterProvider('section', 'S15')
export class RequestHistorySectionProvider extends SectionProvider {
  constructor(
    private readonly resourcing: ResourcingService,
    private readonly accessResolver: AccessResolver,
  ) {
    super();
  }

  async getSection(
    viewerEmployeeId: string,
    subjectEmployeeId: string,
    audience?: ResolvedAudience,
  ): Promise<RequestHistorySectionEntity> {
    const resolved =
      audience ??
      (await this.accessResolver.resolveAudience(
        viewerEmployeeId,
        subjectEmployeeId,
      ));
    const accessLevel = resolved.sections.S15;
    if (!accessLevel || accessLevel === 'none') {
      throw new ForbiddenException('S15 is not visible to this viewer');
    }

    return this.resourcing.buildRequestHistorySection(subjectEmployeeId);
  }
}
