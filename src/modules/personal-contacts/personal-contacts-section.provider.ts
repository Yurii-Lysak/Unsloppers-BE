...
import { PersonalContactsSectionEntity } from './entities/personal-contacts.entity';
import { PersonalContactsService } from './personal-contacts.service';

@Injectable()
@RegisterProvider('section', 'S2')
export class PersonalContactsSectionProvider extends SectionProvider {
  constructor(
  ...
  async getSection(
    ...
    const accessLevel = resolved.sections.S2;
    if (accessLevel === 'none') {
      throw new ForbiddenException('S2 is not visible to this viewer');
    }

    return this.personalContacts.buildPersonalContactsSection(
      subjectEmployeeId,
    );
  }
}
